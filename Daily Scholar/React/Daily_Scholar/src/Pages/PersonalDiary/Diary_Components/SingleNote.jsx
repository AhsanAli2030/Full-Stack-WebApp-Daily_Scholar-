...
const SingleNote = ({ id }) => {
  ...
  let getSingleDiaryNote = async () => {
    let responseFromSingleDiaryNote = await fetch(
      `http://127.0.0.1:8000/personal-diary/single-diary-note/${id}`
    );
  ...
  const [activeComponentSingle, setActiveComponentSingle] =
    useState("update_component");

  let updateDiaryNote = async () => {
    try {
      await fetch(
        `http://127.0.0.1:8000/personal-diary/single-diary-note/${id}/update`,
        {
          method: "PUT",
  ...
  let deleteDiaryNote = async () => {
    try {
      await fetch(
        `http://127.0.0.1:8000/personal-diary/single-diary-note/${id}/delete`,
        {
          method: "DELETE",
          headers: { "Content-Type": "application/json" },
        }
  ...
  const [boolianForNow, setBoolianFornow] = useState(false);
  useEffect(() => {
    setBoolianFornow(true);
  });

  const renderComponent = () => {
    switch (activeComponentSingle) {
          ...
          <>
            {singleNote && ( // Check if singleNote is not null before accessing its properties
              <div id="dates">
                ...
                <div
                  id="button"
                  onClick={() => setActiveComponentSingle("Directory_list")}
                >
                ...
                <div className="updated-last same">
                  ...
                  <br />
                  <span>Date :</span>{" "}
                  <span>{`${dayUpdate} / ${monthUpdate} / ${yearUpdate}`}</span>{" "}
                  <br />
                  <span>Time :</span>{" "}
                  <span>{`${hourUpdate}-${minuteUpdate}-${secUpdate}`}</span>
                </div>
              </div>
            )}
